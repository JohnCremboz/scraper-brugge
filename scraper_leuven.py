"""
Scraper voor PDF-documenten van besluitvorming.leuven.be

Gebruik:
    uv run python scraper_leuven.py --lijst-organen
    uv run python scraper_leuven.py --orgaan "Gemeenteraad" --output pdfs --maanden 12
    uv run python scraper_leuven.py --orgaan "Gemeenteraad" --notulen --maanden 24
    uv run python scraper_leuven.py --orgaan "College van burgemeester en schepenen" --output cbs --maanden 6
    uv run python scraper_leuven.py --alle --maanden 3
"""
...
def toon_organen():
    ...
    if not organen:
        ...
        return
    print("\nBeschikbare organen op besluitvorming.leuven.be:")
    print("-" * 50)
    for org in organen:
        print(f"  - {org['naam']}")
...
def scrape(orgaan: str | None, output_map: str, maanden: int,
    ...
    print(f"\n{'='*60}")
    print(f"  Scraper: besluitvorming.leuven.be")
...
def main():
    parser = argparse.ArgumentParser(
        description="Scraper voor PDF-documenten van besluitvorming.leuven.be",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Voorbeelden:
  uv run python scraper_leuven.py --lijst-organen
  uv run python scraper_leuven.py --orgaan "Gemeenteraad" --maanden 12
  uv run python scraper_leuven.py --orgaan "Gemeenteraad" --notulen --maanden 24
  uv run python scraper_leuven.py --orgaan "College van burgemeester en schepenen" --output cbs --maanden 6
  uv run python scraper_leuven.py --alle --maanden 3 --agendapunten
        """
    )
    ...
    parser.add_argument("--output", "-d", type=str, default="pdfs_leuven",
        help="Uitvoermap (standaard: pdfs_leuven)")
