    ...
    for listing_pad, taak_dir in listing_taken:
        ...
        if kaart_klassen:
            ...
            for kaart in soup.find_all(
                ...
                for a in kaart.find_all("a", href=True):
                    ...
                    if doc_url in gezien:
                        continue
                    gezien.add(doc_url)
                    doc_naam = a.get_text(strip=True) or Path(urlparse(doc_url).path).stem
                    if document_filter:
                        if (document_filter.lower() not in doc_naam.lower() and
                                document_filter.lower() not in doc_url.lower()):
                            continue
                    alle_pdfs.append({"url": doc_url, "naam": doc_naam})
        else:
            ...
            if jaar_re:
                soup = BeautifulSoup(html, "lxml")
                jaar_urls: list[str] = []
                gezien_jaar: set[str] = set()
                for a in soup.find_all("a", href=True):
                    href = a["href"]
                    parsed = urlparse(href)
                    pad = parsed.path
                    if jaar_re.search(pad) and pad not in gezien_jaar:
                ...
                for pagina_nr in range(1, pagina_max + 1):
                    r = await _get(f"{listing_url}{sep}page={pagina_nr}")
                    if r and r.status_code == 200:
        ...
        # Bouw docs-lijst met hint als naam, download parallel
        docs = [
            {
                "url": pdf["url"],
                "naam": sanitize_filename(Path(urlparse(pdf["url"]).path).name) or pdf["naam"],
            }
            for pdf in alle_pdfs
        ]
        taak_resultaten = await async_download_documents_parallel(
            SESSION, _config, docs, taak_dir, require_pdf=True,
        )
